...
export default function WelcomeLoader() {
  ...
  const [statusText, setStatusText] = useState("Calibrating Academic RAG Pipeline...");
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setProgress((prev) => {
        ...
        const next = prev + 2;
        if (next === 30) setStatusText("Indexing Provincial Board Textbooks & Past Papers...");
        if (next === 65) setStatusText("Initializing Qdrant Multi-Hop Vector Index...");
        if (next === 90) setStatusText("Synthesizing Personalized Mentorship Workspace...");
        return next;
      });
    }, 38);
  ...
  return (
    <div className="min-h-screen w-full bg-[#ededed] flex items-center justify-center p-4 selection:bg-[#18181b] selection:text-white select-none">
      {/* Elevated White Card Container (Matching Reference Design) */}
      <motion.div
      ...
      >
        ...
        <p className="mt-1 text-xs text-[#71717a] max-w-xs leading-relaxed">
          Curriculum-grounded AI academic mentor for FSc, College & Entry Test mastery.
        </p>
        ...
        <div className="mt-5 pt-3 border-t border-[#f4f4f5] flex items-center justify-center gap-4 text-[10px] text-[#a1a1aa]">
          <span className="flex items-center gap-1">
            <BookOpen className="w-3 h-3 text-[#18181b]" /> Board Grounded
          </span>
          <span className="flex items-center gap-1">
            <Compass className="w-3 h-3 text-[#18181b]" /> Entry Test Ready
          </span>
        </div>
      </motion.div>
    </div>
